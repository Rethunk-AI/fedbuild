...
| F5 | **Devbox variant behaviour preserved byte-for-byte** — moving files into `variants/devbox/` must not change RPM or image hashes when built from the same git SHA. | Pre-refactor `bastion-vm-firstboot-*.rpm` SHA256 matches post-refactor SHA256 (SOURCE_DATE_EPOCH-controlled). |
...
| F7 | **`extra-rpms/` pickup** — a variant may declare a directory of upstream RPMs to fold into its local yum repo alongside the fedbuild-built firstboot RPM. Operator populates this before `make image`. Missing directory is fine for variants that don't need it. | bastion-edge variant `extra-rpms/` is the pickup point for the output of `yarn release:rpm` in the bastion-edge source repo. |
...
| F11 | **Per-variant size + boot-time baselines** — `tests/<variant>/size.baseline` and `tests/<variant>/boot-time.baseline` — devbox and bastion-edge diverge wildly (devbox ~2 GiB with Homebrew + toolchains, edge ~800 MiB minimal). | `make VARIANT=bastion-edge check-size` compares against the edge baseline, not devbox's. |
