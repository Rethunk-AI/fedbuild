...
| F5 | **Devbox variant behaviour preserved byte-for-byte** — moving files into `variants/devbox/` must produce an identical RPM when built with the **pre-refactor `SOURCE_DATE_EPOCH`** pinned via env var. This works around the fact that `git mv` advances the path's last-touching-commit ctime (the rename commit), which is the Makefile's default SDE source. | `SOURCE_DATE_EPOCH=1776424334 make VARIANT=devbox rpm` post-refactor produces the same SHA256 as `make rpm` pre-refactor. (`1776424334` is the ctime of the last pre-refactor content commit touching `bastion-vm-firstboot/`.) |
| F5a | **Makefile honours `SOURCE_DATE_EPOCH` env override** — the RPM target must respect an operator-supplied `SOURCE_DATE_EPOCH` rather than always deriving from `git log`. Enables reproducible rebuilds across renames, across clones, and across CI runners. | `sde=$${SOURCE_DATE_EPOCH:-$$(git log ...)}` pattern in the rpm recipe; `SOURCE_DATE_EPOCH=<fixed> make rpm` produces identical output regardless of git state. |
