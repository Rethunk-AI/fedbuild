...
| F6 | **bastion-edge variant added** — `variants/bastion-edge/` with blueprint for Fedora 43 minimal + the `bastion-theatre` + `bastion-theatre-manager` runtime RPMs folded in via `extra-rpms/` (the upstream `bastion-edge` source repo ships two version-matched RPMs, not one). Includes `bastion-edge-firstboot.spec` that stamps `/var/lib/bastion-edge/edge-id` from cidata or `/etc/machine-id`. The systemd unit that fedbuild enables in the image is `bastion-theatre-manager.service` (the service unit shipped by the upstream `bastion-theatre-manager` RPM — there is no `bastion-edge.service`). | `make VARIANT=bastion-edge rpm` builds the firstboot RPM; `make VARIANT=bastion-edge image` (sudo) builds the image; `make VARIANT=bastion-edge smoke` (KVM) boots the image and asserts `systemctl is-active bastion-theatre-manager` returns `active`. |
| F7 | **`extra-rpms/` pickup** — a variant may declare a directory of upstream RPMs to fold into its local yum repo alongside the fedbuild-built firstboot RPM. Operator populates this before `make image`. The `repo` target **must** be extended to sweep `$(VARIANT_DIR)/extra-rpms/*.rpm` into `$(REPODIR)` before `createrepo`. Missing directory is fine for variants that don't need it. | bastion-edge variant `extra-rpms/` is the pickup point for the output of `yarn release:rpm:theatre` + `yarn release:rpm:theatre-manager` in the bastion-edge source repo (two version-matched RPMs); `make VARIANT=bastion-edge repo` includes those RPMs in the createrepo metadata. |
